...
namespace WindowsFormsApp1.CustomExtension
{
    public class TriggerSQL
	{
		string _triggerHeader = @"
			ALTER TRIGGER[dbo].[DML_Products_ChangeLog] ON[dbo].[{{nametable}}]
...
";
		...
		public async Task<string> GenTrigger(string tableName, Dictionary<string, List<string>> model)
        {
			...
			await Task.Run(() => {
				...
				{
					trigger += _triggerbody.Replace(cSharp.ParametersPass, field[i]) + "\n";
				}
